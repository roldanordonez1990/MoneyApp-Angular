...
import { DialogTypes } from '../components/dialogos/dialogos-general'; //importamos la clase interna
...
export class DialogosService {
  ...
  */ 
  abrirDialogCargando() {
    ...
    this.dialog.open(DialogosComponent, this.dialogConfig); // abre el diálogo
  }
  ...
  abrirDialogError(textoDeError: string) {
    ...
    this.dialog.open(DialogosComponent, this.dialogConfig); // abro el diálogo
  }
  ...
  abrirDialogInfo(textoDeInfo: string): Observable<number> {
    ...
    const dialogRef = this.dialog.open(DialogosComponent, this.dialogConfig);
